...
def main() -> None:
    ...
    print(" Trying to validate input 'hello'", log_test.validate({ 1: 'test', 2: 'test'}))
    log_test_list = [{'log_level': 'NOTICE', 'log_message': 'Connection to server'}, {'log_level': 'ERROR', 'log_message': 'Unauthorized access!!'}]
    print(f" Processing data: {log_test_list}")
    log_test.validate(log_test_list)
    log_test.ingest(log_test_list)
    print(log_test.stored_data)
    print(" Extracting 2 values...")
    l = log_test.output()
    print(f" Numeric value {l[0]}: {l[1]}")
    l = log_test.output()
    print(f" Numeric value {l[0]}: {l[1]}")
